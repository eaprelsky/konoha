...
export const ACTIONS: ActionDef[] = [
  ...
  },
  {
    ...
    args: [
      ...
      { name: "idempotency_key", type: "string", required: false, description: "Optional caller deploy request key; effective transaction keys are always scoped by workflow id and deploy version." },
    ],
  ...
  },
  {
    ...
    args: [
      ...
      { name: "documents",   type: "array",  required: false, description: "Document IDs attached to a function element." },
      { name: "systems",     type: "array",  required: false, description: "Information-system adapter bindings attached to a function element." },
      { name: "intent",      type: "string", required: false, description: "Intent/goal text for an intent-based function element." },
      { name: "sub_process_id", type: "string", required: false, description: "Child workflow ID for a subprocess function element." },
      { name: "expected_edit_version", type: "number", required: false, description: "Optional edit-version guard for conflict detection." },
      { name: "expected_deploy_version", type: "number", required: false, description: "Optional deploy-version guard for conflict detection." },
    ],
    implementation: { kind: "direct", note: "Updates an existing element through action-executor with validation and version guards." },
    autonomy: "confirm",
    audited: true,
  },
  {
    ...
    implementation: { kind: "direct", note: "Removes an element and connected flow edges through action-executor with validation and version guards." },
    autonomy: "confirm",
    audited: true,
  },
  ...
  // ── Trigger ───────────────────────────────────────────────────────────────
  {
    ...
    args: [
      ...
      { name: "kind",        type: "string", required: true,  description: "Trigger kind: timer | message | condition | system | manual | delay_after." },
      { name: "config",      type: "object", required: false, description: "Kind-specific trigger configuration." },
      { name: "expected_edit_version", type: "number", required: false, description: "Optional edit-version guard for conflict detection." },
      { name: "expected_deploy_version", type: "number", required: false, description: "Optional deploy-version guard for conflict detection." },
    ],
    implementation: { kind: "direct", note: "Sets event trigger config through action-executor with validation and version guards." },
    autonomy: "confirm",
    audited: true,
  },
  {
