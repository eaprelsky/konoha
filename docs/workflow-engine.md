...
For deployed start triggers, `workflow.deploy` owns subscription materialization through the workflow deployment service. Direct workflow create/update and public subscription APIs must not create `instance_id="new"` start-event subscriptions; that identifier is reserved for deploy-managed subscriptions with deployment metadata. Runtime/intermediate event subscriptions still use the event manager with the running `case_id` as `instance_id`. The transaction order is fixed: validation, executable workflow commit, deployed snapshot save, subscription diff materialization, then persisted deploy receipt. Deploy receipts include a `transaction` object with `transaction_id`, deployment-scoped `idempotency_key`, optional `caller_idempotency_key`, `commit_order`, record keys, and retry policy, plus `deployment_id`, `deploy_version`, and per-event `operation_key`/`idempotency_key` entries for `created`, `cancelled`, `unchanged`, and `failed` subscription operations. Each completed or blocked deploy also writes a durable deploy record at `workflow:deploy-record:<workflow_id>:v<deploy_version>` with the subscription diff, rollback evidence, and failure details needed by subscription reconciliation follow-ups.
