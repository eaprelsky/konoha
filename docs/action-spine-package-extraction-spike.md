...
Issue #744 recorded extraction readiness evidence for #618 before the package
scaffold started. #684, #685, and #686 are now accepted and closed as
`state:done`, so #618 owns the initial private scaffold in
`packages/action-spine`. This document now separates completed scaffold checks
from remaining future extraction checks.
...
`docs/action-spine-extraction-closure-report.json`; that report was parent
evidence only at #684 closure time. The accepted #685/#686 gates permit the #618
scaffold, but runtime adapters still remain host-owned.

## Current State

| Layer | Current files | Status |
| --- | --- | --- |
| Generic core shapes | `src/action-spine/core-types.ts`, `packages/action-spine/src/core-types.ts` | Mirrored into the package scaffold. No Konoha action IDs/scopes. |
| Generic host ports | `src/action-spine/ports.ts`, `packages/action-spine/src/ports.ts` | Mirrored into the package scaffold. Defines executor, audit, autonomy, HTTP, and MCP seams. |
...
## Completed #618 Scaffold Outcome

The accepted gates allow the first private scaffold to exist. The scaffold may
own:
...
- package-local registry and bridge helpers that execute through injected host
  ports.

It must not move Konoha runtime behavior, publish a public package, or make
Konoha action IDs/scopes generic package vocabulary.
...
## Required Injectable Dependencies For Host Adoption

Before Konoha replaces its host MCP/HTTP adapters with package-backed adapters,
the package-facing API must inject these host dependencies instead of importing
Konoha modules:
...
- [x] #684 accepted: extraction readiness umbrella is closed.
- [x] #685 accepted: constructor -> deploy -> run -> assigned work item golden path passes.
...
- [x] Package-local MCP bridge has registry/executor-injected tests that do not
  import Konoha vocabulary from package code.
- [x] Package-local HTTP adapter has a port-injected test covering auth,
  autonomy, audit, endpoint fallback, and direct execution through injected host
  callbacks. Konoha's `src/act-envelope.ts` adoption remains a future check.
- [x] Package scaffold has no Konoha runtime, route, storage, workflow, agent, or
  concrete action vocabulary imports.
- [x] Konoha still passes `action-surface-report.ts --check`;
  `docs/action-surface.json` remains host-generated.

Future checks before broader extraction or publishing:

- [ ] Konoha `src/mcp-action-bridge.ts` adoption uses injected package bridge
  dependencies without importing Konoha internals from package code.
- [ ] Konoha `src/act-envelope.ts` adoption uses an injected package route
  factory while preserving route-auth, audit, autonomy, endpoint fallback, and
  direct executor regressions.
- [ ] Public package publishing has explicit versioning, changelog, and release
  policy.
- [ ] Any workflow/case/runtime-adjacent adoption proves it cannot bypass #812,
  or continues listing #812 as open/not waived.

## Remaining Guardrails For #618
...
- Do not close, unpause, or waive #812 from this extraction work.

## Minimum Checks For A Future Host-Adoption Commit
