...
| `hinata` | Optional QA executor | `optional_on_demand` | `persistent_interactive` | Reviewer-requested QA | none | `konoha-qa.slice` via `agent-managed@.service` | `agent-managed@hinata.service` / `hinata` / `agent-watchdog-lifecycle.service` | on demand | Missing tmux is expected unless reviewer activates QA. |
