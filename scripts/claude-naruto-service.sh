...
while true; do
    ...
    if [ -f "$MCP_CONFIG" ]; then
        ...
        tmux send-keys -t "$SESSION" "claude --model claude-sonnet-4-6 --dangerously-skip-permissions" Enter
    fi

    /home/ubuntu/scripts/wait-for-prompt.sh "$SESSION" 90
    tmux send-keys -t "$SESSION" BTab
    sleep 1
    tmux send-keys -t "$SESSION" 'Прочитай CLAUDE.md и выполни startup sequence.' Enter
