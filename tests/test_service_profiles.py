...
AGENT_API_SERVICE = Path(__file__).resolve().parents[1] / "scripts" / "agent-api-service.sh"
...
def test_prod_core_is_lean_always_on_profile():
    ...
    assert {"kakashi", "shino"}.issubset(profile.disabled_lifecycle_agents)
...
def test_prod_full_enables_sdd_lane_without_specialist_autostart():
    ...
    assert "kakashi" not in profile.autostart_agents
    assert "kakashi" not in profile.disabled_lifecycle_agents
    assert "shino" not in profile.autostart_agents
    assert "shino" in profile.disabled_lifecycle_agents
    assert "agent-watchdog-kakashi.service" in profile.required_services
    assert "agent-kakashi.service" in profile.optional_services
...
def test_staging_and_qa_profiles_keep_external_connectors_disabled():
    ...
    assert {"kakashi", "shino"}.issubset(staging.disabled_lifecycle_agents)
    assert "kakashi" in qa.disabled_lifecycle_agents
    assert "shino" not in qa.disabled_lifecycle_agents
...
def test_profile_catalog_json_has_no_duplicate_services():
    ...
    for profile_id, profile in raw["profiles"].items():
        ...
        disabled_agents = profile["disabled_lifecycle_agents"]
        ...
        assert len(disabled_agents) == len(set(disabled_agents)), profile_id
...
def test_agent_api_service_respects_disabled_lifecycle_profiles():
    script = AGENT_API_SERVICE.read_text(encoding="utf-8")

    assert "lifecycle_disabled_by_profile" in script
    assert "disabled_lifecycle_agents" in script
    assert "not requesting /start" in script
    assert "KONOHA_ENABLE_DISABLED_LIFECYCLE_AGENTS" in script
