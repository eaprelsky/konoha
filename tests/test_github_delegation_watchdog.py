...
class GitHubDelegationWatchdogTest(unittest.TestCase):
    def test_architect_label_routes_to_shikadai(self):
        module = load_module("shikadai", "agent:shikadai", "shikadai-batch")

        self.assertTrue(module.is_delegated_issue(issue_with_labels("agent:shikadai")))
        self.assertFalse(module.is_delegated_issue(issue_with_labels("agent:kakashi")))
        self.assertEqual(
            module.task_text(issue_with_labels("agent:shikadai")),
    ...
    def test_skip_labels_prevent_dispatch(self):
        ...
        self.assertFalse(
            module.is_delegated_issue(issue_with_labels("agent:shikadai", "state:blocked"))
        )
        self.assertFalse(
            module.is_delegated_issue(issue_with_labels("agent:shikadai", "state:done"))
        )

    def test_redispatch_requires_batch_label_and_cooldown(self):
        module = load_module("shikadai", "agent:shikadai", "shikadai-batch")
        issue = issue_with_labels("agent:shikadai", "shikadai-batch")
